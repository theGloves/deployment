...
if shard_count == 1:
    ...
    cmd = "cd dist;./tm-bench-tendermint -s 500 " + " -r " + r + " -T " + T + " 0S1.tendermint:26657"
    print(cmd)
else:
    Send_shard = ""  # 要发往哪些分片
    ...
    for i in range(0, shard_count):
        ...
        time.sleep(0.01)
        if i == 0:
            shard_node = "0S" + str(random.randint(
                1, int(send_count))) + ".tendermint:26657"
            continue

        shard_node = shard_node + "," + shard_name + "S" + str(
            random.randint(1, int(send_count))) + ".tendermint:26657"
